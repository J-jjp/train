...
class LeggedRobotCfg(BaseConfig):
    class env:
        ...
        n_scan = 132    #扫描数据的数量
        n_priv_latent = 4 + 1 + 12 + 12 + 6 #私有潜在变量的数量
        n_proprio = 46#自有感知
        history_len = 10#长期历史

        num_observations = n_proprio + n_scan + history_len*n_proprio + n_priv_latent #观察的数量
        num_privileged_obs = None # if not None a priviledge_obs_buf will be returned by step() (critic obs for assymetric training). None is returned otherwise 
        num_actions = 12    #输出维度
        env_spacing = 3.  # 环境间距not used with heightfields/trimeshes 
        send_timeouts = True # send time out information to the algorithm
        episode_length_s = 20 # episode length in seconds

        history_encoding = True

        include_foot_contacts = True    #是否包括足部接触信息
        contact_buf_len = 100   #接触缓冲区的长度
    ...
    class terrain:
        ...
        horizontal_scale = 0.1  # [m]设置地形在水平方向上的缩放比例
        vertical_scale = 0.005  # [m]设置地形在垂直方向上的缩放比例
        border_size = 25  # [m] 表示地形的边界大小
        curriculum = True#课程学习
        static_friction = 1.0#设置地面上的静摩擦系数
        dynamic_friction = 1.0# 设置动态摩擦系数
        restitution = 0.#启用或禁用地形高度测量
        # rough terrain only:
    ...
    class asset:
        ...
        foot_name = "None"  #脚体的名称，用于为身体状态和接触力张量编制索引
