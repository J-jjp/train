...
import cv2
import os
from configs.go1_constraint import Go1ConstraintHimRoughCfg, Go1ConstraintHimRoughCfgPPO
...
def play(args):
    ...
    '/home/jiaojunpeng/isaac/LocomotionWithNP3O-masterV5/LocomotionWithNP3O-master/logs/rough_go2_constraint/Dec03_15-07-55_test_barlowtwins/model_1500.pt'))#《---------------------调用的网络模型doghome

    #if 1:#full 
    policy.load_state_dict(model_dict['model_state_dict'])
    policy.half()
    policy = policy.to(env.device)
    torch.save(policy,'go1.pt',)
    print('*****************')
    print(policy)
    #else:#origin
    # policy.load_state_dict(model_dict['model_state_dict'])
