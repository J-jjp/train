...
class LeggedRobot(BaseTask):
    ...
    def _init_buffers(self):
        """ Initialize torch tensors which will contain simulation states and processed quantities
            isaac gym order: FL0 FR1 RL2 RR3
                0 FL_hip_joint 3
                1 FL_thigh_joint 4
                2 FL_calf_joint 5
                3 FR_hip_joint 0 
            ...
            unitree go2 sdk order:
                ...
                7 RL_thigh_joint 10
                8 RL_calf_joint 11
            tinymal sdk order: FR1 RR3 FL0 RL2 
                3 FR_hip_joint 0
                4 FR_thigh_joint 1
                5 FR_calf_joint 2
                9 RR_hip_joint 3
        ...
        self.last_feet_z = torch.zeros((self.num_envs, 4), device=self.device)#0.05
    ...
    def compute_observations(self):#guance
        ...
        noise_vec = torch.cat((torch.ones(3) * noise_scales.ang_vel * noise_level,
                               ...
                               torch.zeros(3+2*0),#need fix for sin cos
                               torch.ones(
    ...
    def _reward_orientation_eular(self):
        ...
        return quat_mismatch#(quat_mismatch + orientation) / 2.
    ...
    def _reward_default_joint_pos(self):
        ...
        roll_FL = joint_diff[:, 0]
        roll_FR = joint_diff[:, 3]
        roll_RL = joint_diff[:, 6]
        roll_RR = joint_diff[:, 9]
        roll = torch.abs(roll_FL)+torch.abs(roll_FR)+torch.abs(roll_RL)+torch.abs(roll_RR)
        return torch.exp(-roll * 10)+ 0.01 * torch.norm(joint_diff, dim=1)
    ...
    def _reward_joint_ref_pos(self):# 期望关节角度轨迹
        ...
        r *= (torch.norm(self.commands[:, :2], dim=1) > self.cfg.rewards.command_dead)
        return r
    ...
    def _reward_tracking_lin_vel(self): # 在误差较小时奖励接近于1，而在误差较大时奖励接近于0
        # Reward tracking specified linear velocity command
        error = self.commands[:, :2] - self.base_lin_vel[:, :2]
        error *= 1./(1. + torch.abs(self.commands[:, :2]))
        error = torch.sum(torch.square(error), dim=1)
        return 0.5 + 0.5 * torch.exp(-error/self.cfg.rewards.tracking_sigma) # tracking_sigma == 0.5 标准差参数，控制着指数函数的衰减速度
    ...
    def _reward_feet_clearance_inter(self):
        """
        ...
        rew_pos = torch.abs(self.feet_height - abs(self.cfg.rewards.clearance_height_target)) < 0.01
        rew_pos = torch.sum(rew_pos , dim=1)
        self.feet_height *= ~contact#clear height
        cur_footvel_translated = self.feet_vel - self.root_states[:, 7:10].unsqueeze(1)
        footvel_in_body_frame = torch.zeros(self.num_envs, len(self.feet_indices), 3, device=self.device)
        for i in range(len(self.feet_indices)):
            footvel_in_body_frame[:, i, :] = quat_rotate_inverse(self.base_quat, cur_footvel_translated[:, i, :])

        height_error = torch.square(self.feet_height - abs(self.cfg.rewards.clearance_height_target)).view(self.num_envs, -1)
        foot_leteral_vel = torch.sqrt(torch.sum(torch.square(footvel_in_body_frame[:, :, :2]), dim=2)).view(self.num_envs, -1)
    ...
    def _reward_stand_still(self):
        ...
        return torch.sum(torch.abs(self.dof_pos - self.default_dof_pos), dim=1) * (torch.norm(self.commands[:, :2], dim=1) <  self.cfg.rewards.command_dead)
    ...
    def _reward_feet_distance(self):
        ...
        foot_dist1 = torch.norm(foot_pos[:, 0, :] - foot_pos[:, 3, :], dim=1)#*torch.logical_or(contact_filt[0], contact_filt[3]) 
        foot_dist2 = torch.norm(foot_pos[:, 1, :] - foot_pos[:, 2, :], dim=1)#*torch.logical_or(contact_filt[1], contact_filt[2]) 
        #print(foot_dist1[0])
        fd = self.cfg.rewards.min_dist
        max_df = self.cfg.rewards.max_dist
        d_min1 = torch.clamp(foot_dist1 - fd, -0.5, 0.)
        d_max1 = torch.clamp(foot_dist1 - max_df, 0, 0.5)
        d_min2 = torch.clamp(foot_dist2 - fd, -0.5, 0.)
        d_max2 = torch.clamp(foot_dist2 - max_df, 0, 0.5)
        return (torch.exp(-torch.abs(torch.min(d_min1,d_min2)) * 100) + torch.exp(-torch.abs(torch.max(d_max1,d_max2)) * 100)) / 2
